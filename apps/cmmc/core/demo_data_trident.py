"""Trident Defense Systems demo data — mirrors Apex structure but with unified Trident branding across all frameworks."""
...
def _evidence_file_key(control_id: str, filename: str) -> str:
    safe = "".join(c for c in filename if c.isalnum() or c in "._- ")[:200]
    return f"{control_id}_{safe}"


def _make_evidence(
    filename: str,
    content: str,
    upload_date: str = "2026-07-01 14:30",
) -> Dict[str, Any]:
...
def _demo_evidence_map() -> Dict[str, list[Dict[str, Any]]]:
    files: list[tuple[str, str, str]] = [
        ("AC.L2-3.1.1", "Access Control Policy v2.1.pdf", (
            "TRIDENT DEFENSE SYSTEMS — Access Control Policy v2.1\n"
            "Effective: 2026-01-15 | Last Review: 2026-Q1\n\n"
            "1. Purpose: Define access control requirements for the CUI enclave.\n"
            "2. Scope: All users, devices, and services accessing CUI data.\n"
            "3. Policy: Least privilege, need-to-know, quarterly access reviews.\n"
            "4. Enforcement via Entra ID Conditional Access and SharePoint permissions.\n"
            "Approved: Jordan Lee, VP Operations"
        )),
        ("AC.L2-3.1.1", "MFA Conditional Access Configuration.png", (
            "[SCREENSHOT] Microsoft Entra Admin Center > Conditional Access\n"
            "Policy: CA-MFA-ALL-CUI\n"
            "Status: Enabled\n"
            "Users: All CUI-enabled users\n"
            "Cloud apps: SharePoint CUI library, Azure Management\n"
            "Grant: Require multi-factor authentication\n"
            "Session: Sign-in frequency 4 hours"
        )),
        ("AC.L2-3.1.16", "Rules of Behavior signed.pdf", (
            "TRIDENT DEFENSE SYSTEMS — Rules of Behavior\n"
            "Acknowledged by: Engineering Department\n"
            "Date: 2026-01-22\n\n"
            "I acknowledge my responsibility to protect CUI data, use authorized systems only,\n"
            "report security incidents immediately, and comply with all Trident security policies.\n"
            "Signed: [Employee roster on file per role]"
        )),
        ("SC.L2-3.13.1", "Network Diagram v2.pdf", (
            "TRIDENT CUI ENCLAVE — Network Topology v2\n"
            "Updated: 2026-06-15\n\n"
            "[Internet] → [FortiGate FG-60F / VPN] → [Entra ID / SharePoint GCC]\n"
            "                                  → [Managed Windows 11 Endpoints (Intune)]\n"
            "Segmentation: CUI SharePoint site isolated by sensitivity labels\n"
            "Remote: Entra App Proxy + MFA\n"
            "Monitoring: Defender for Business, SIEM (FortiGate logs)"
        )),
        ("SC.L2-3.13.1", "Data Flow Diagram - Trident CUI.pdf", (
            "TRIDENT CUI DATA FLOW\n"
            "1. Engineer creates CUI drawing in SolidWorks\n"
            "2. Exported to SharePoint CUI library (TLS 1.2+)\n"
            "3. Entra ID authentication + Conditional Access check\n"
            "4. Sensitivity label applied automatically\n"
            "5. Approved PMs review via SharePoint (no local copies)\n"
            "6. Audit log entries generated for each access"
        )),
        ("AT.L2-3.2.1", "Security Awareness Training Q1-2026.xlsx", (
            "TRIDENT SECURITY AWARENESS TRAINING — Q1 2026\n"
            "Course: CUI Handling v4 | Date: 2026-02-15\n"
            "Total enrolled: 48 | Completed: 45 | Pass rate: 100%\n"
            "Trainer: Sam Rivera (ISO)\n"
            "Topics: CUI marking, phishing, clean desk, reporting\n"
            "Outstanding: 3 users (on leave) — catch-up by 2026-04-01"
        )),
        ("IA.L2-3.5.3", "Entra ID MFA Compliance Report.pdf", (
            "Microsoft Entra ID — MFA Compliance Report\n"
            "Generated: 2026-06-28\n\n"
            "Global Admins: MFA enforced (CA-MFA-PRIV)\n"
            "CUI Users: MFA required for SharePoint access\n"
            "Legacy Auth: Blocked at tenant level\n"
            "Exceptions: 2 legacy workstations (see POA&M IA.L2-3.5.3)\n"
            "Compliance: 96% — remediation plan active"
        )),
        ("IA.L2-3.5.3", "Conditional Access Policy CA-MFA-PRIV.png", (
            "[SCREENSHOT] Microsoft Entra Admin Center\n"
            "Policy: CA-MFA-PRIV\n"
            "Assignment: All privileged roles\n"
            "Grant: Require MFA + Compliant device\n"
            "Session: Token lifetime 8 hours\n"
            "Report-only: No — Enabled: Yes"
        )),
        ("CM.L2-3.4.1", "Baseline Configuration Win11-CUI-v4.pdf", (
            "TRIDENT — Windows 11 CUI Baseline v4\n"
            "Intune Profile: Win11-CUI-v4\n"
            "Assigned: 27 of 45 endpoints\n\n"
            "BitLocker: Enabled (AES-256)\n"
            "Defender: Real-time protection ON\n"
            "Firewall: Domain/Private profiles enabled\n"
            "AppLocker: Block executables from Temp/AppData\n"
            "Local Admin: Removed via Intune"
        )),
        ("CA.L2-3.12.4", "System Security Plan v2.4.docx", (
            "TRIDENT DEFENSE SYSTEMS\n"
            "System Security Plan — Trident CUI Enclave v2.4\n"
            "Date: 2026-05-01\n\n"
            "1. System Description: Cloud-first CUI enclave using Microsoft GCC.\n"
            "2. Boundary: Entra ID, Intune endpoints, SharePoint CUI, FortiGate VPN.\n"
            "3. Applicable Standards: NIST SP 800-171 Rev 2, CMMC Level 2.\n"
            "4. Controls: 110 scoped controls; all implemented per Trident policies.\n"
            "5. POA&M: 5 open items tracked in separate document."
        )),
        ("CA.L2-3.12.2", "POA&M Q2-2026.xlsx", (
            "TRIDENT POA&M — Q2 2026\n"
            "Open findings: 4 | Critical: 0 | High: 2 | Medium: 2\n\n"
            "IA.L2-3.5.3 | MFA gaps on legacy workstations | Due: 2026-09-30\n"
            "SC.L2-3.13.11 | FIPS attestation | Due: 2026-08-01\n"
            "IR.L2-3.6.1 | Tabletop exercise | Due: 2026-09-30\n"
            "CM.L2-3.4.1 | Intune baseline rollout | Due: 2026-09-30"
        )),
        ("AU.L2-3.3.1", "Entra ID Audit Log Sample.csv", (
            "Timestamp,User,Application,Activity,IP Address,Status\n"
            "2026-06-28T08:15:00Z,jlee@trident.onmicrosoft.com,SharePoint Online,FileDownloaded,203.0.113.42,Success\n"
            "2026-06-28T08:20:00Z,akim@trident.onmicrosoft.com,Azure Portal,RoleAssignment,203.0.113.17,Success\n"
            "2026-06-28T09:00:00Z,srivera@trident.onmicrosoft.com,Entra Admin,PolicyChange,198.51.100.8,Success\n"
            "2026-06-28T09:30:00Z,jlee@trident.onmicrosoft.com,SharePoint Online,SensitivityLabelApplied,203.0.113.42,Success\n"
            "2026-06-27T22:15:00Z,svc-cui-sync@trident.onmicrosoft.com,Entra ID,SignIn,10.0.0.15,Failure"
        )),
        ("AU.L2-3.3.1", "SIEM Rule Export - Privileged Access.json", (
            '{\n  "rule_name": "Privileged Role Assignment Alert",\n'
            '  "severity": "High",\n  "source": "Entra ID Audit Logs",\n'
            '  "condition": "activity = RoleAssignment AND status = Success",\n'
            '  "response": "Notify ISO within 15 minutes",\n'
            '  "enabled": true\n}'
        )),
        ("RA.L2-3.11.2", "Vulnerability Scan Report 2026-06.pdf", (
            "Tenable.io Scan Report — Trident CUI Enclave\n"
            "Scan Date: 2026-06-15\n\n"
            "Total Findings: 23\n"
            "Critical: 0\n"
            "High: 2 (KB 5048652 missing on 2 endpoints)\n"
            "Medium: 5\n"
            "Low: 16\n\n"
            "Compliance: 91% of endpoints fully patched\n"
            "Scan coverage: 42 of 45 endpoints (3 off-network)"
        )),
        ("SC.L2-3.13.11", "FIPS 140-2 Attestation Bundle.pdf", (
            "MICROSOFT CLOUD — FIPS 140-2 ATTESTATION\n"
            "Service: Azure / Microsoft 365 GCC\n\n"
            "Azure SSE: FIPS 140-2 validated (Cert. #3875)\n"
            "BitLocker: FIPS 140-2 validated (Cert. #3938)\n"
            "TLS 1.2: FIPS 140-2 compliant\n\n"
            "Status: Attestation downloaded from Microsoft Service Trust Portal\n"
            "Date: 2026-06-20\n"
            "Org responsibility: Verify endpoint crypto configuration"
        )),
        ("IR.L2-3.6.1", "Incident Response Plan v2.1 draft.pdf", (
            "TRIDENT DEFENSE SYSTEMS — INCIDENT RESPONSE PLAN v2.1 (DRAFT)\n\n"
            "1. Preparation: Roles defined; tools deployed (Defender, SIEM).\n"
            "2. Detection: Helpdesk / Defender alerts / User report.\n"
            "3. Analysis: ISO triages within 1 hour.\n"
            "4. Containment: Disconnect affected system; revoke sessions.\n"
            "5. Eradication: Remove threat; patch vulnerability.\n"
            "6. Recovery: Restore from clean backup.\n"
            "7. Post-mortem: Report within 30 days.\n"
            "Note: Tabletop exercise pending (POA&M IR.L2-3.6.1)"
        )),
        ("SI.L2-3.14.1", "Patch Compliance Report June 2026.pdf", (
            "Intune Patch Compliance — June 2026\n"
            "Baseline: Win11-CUI-v4\n\n"
            "Total endpoints: 45\n"
            "Compliant: 42 (93%)\n"
            "Non-compliant: 3 (pending maintenance window)\n\n"
            "Missing patches:\n"
            "KB5048652 — 2 endpoints (restart pending)\n"
            "KB5048698 — 1 endpoint (off-network >14 days)\n\n"
            "Average patch latency: 8 days"
        )),
        ("PS.L2-3.9.1", "Personnel Screening Record Sample.pdf", (
            "TRIDENT HR — PERSONNEL SCREENING RECORD\n"
            "Employee: [Redacted per Privacy Act]\n"
            "Position: CUI Engineer\n"
            "Clearance: ITAR-compliant background check\n"
            "Screening date: 2026-01-10\n"
            "Result: Pass\n"
            "Re-screening: Every 5 years\n"
            "Notes: DD254 on file for contract TRIDENT-2026-001"
        )),
        ("PE.L2-3.10.1", "Physical Access Log Sample.xlsx", (
            "TRIDENT HQ — PHYSICAL ACCESS LOG\n"
            "Date,Visitor Name,Host,Badge ID,Time In,Time Out\n"
            "2026-06-20,John Contractor,A. Kim,VIS-042,09:00,16:30\n"
            "2026-06-21,MSP Audit Team,S. Rivera,AUD-001,08:30,17:00\n"
            "2026-06-22,Vendor Support,A. Kim,VIS-043,10:00,14:00"
        )),
        ("MA.L2-3.7.1", "Maintenance Log Q2-2026.xlsx", (
            "TRIDENT MAINTENANCE LOG — Q2 2026\n"
            "Date,Asset,Type,Vendor,Approved By,Notes\n"
            "2026-04-15,FortiGate FG-60F,Firmware Update,Fortinet,J. Lee,Scheduled downtime 22:00-23:00\n"
            "2026-05-20,Azure GCC,Service Update,Microsoft,J. Lee,No user impact\n"
            "2026-06-10,Endpoints,Patch Tuesday,Intune,S. Rivera,Deployed to pilot group first"
        )),
        ("MP.L2-3.8.3", "Media Sanitization Record 2026.xlsx", (
            "TRIDENT MEDIA SANITIZATION — 2026\n"
            "Date,Asset,Method,Certification,Witness\n"
            "2026-03-15,Retired laptop TRIDENT-LT-031,DoD 5220.22-M wipe,Passed,A. Kim\n"
            "2026-05-01,USB drive TRIDENT-USB-007,Cryptographic erase,Passed,A. Kim\n"
            "2026-06-01,Server SSD TRIDENT-SSD-002,Physical destruction,Passed,S. Rivera"
        )),
    ]

    evidence_map: Dict[str, list[Dict[str, Any]]] = {}
    restored: Dict[str, bytes] = {}
    for cid, fname, content in files:
        ev = _make_evidence(fname, content)
        evidence_map.setdefault(cid, []).append(ev)
        key = _evidence_file_key(cid, fname)
        restored[key] = ev["data_bytes"]
    return evidence_map, restored

DEMO_ORG_NAME = "Trident Defense Systems"
...
_FAMILY_NARRATIVE = {
    "Access Control": (
        "Trident implements {name} within the CUI enclave using Microsoft Entra ID groups, SharePoint permission "
    ...
    "Awareness and Training": (
        ...
        "within 30 days of hire. {name} is documented in LMS course TRIDENT-CUI-AT. "
    ...
    "Incident Response": (
        "Trident maintains Incident Response Plan v2.1 (draft) covering {name}. Reporting path: user → helpdesk → "
    ...
    "Physical Protection": (
        "CUI processing occurs at Trident HQ (Badge access) and remote telework on approved managed devices only. "
...
def get_demo_org_profile() -> Dict[str, str]:
    return {
        ...
        "system_name": "Trident CUI Enclave (Microsoft 365 GCC + Managed Endpoints)",
        "system_unique_id": "TRIDENT-ENCLAVE-001",
        "system_description": (
            "Trident Defense Systems engineers and manufactures subassemblies for DoW programs. "
            "The Trident CUI Enclave is a cloud-first environment where controlled technical data (CUI) "
        ...
        "architecture_summary": (
            ...
            "in SharePoint site collection TRIDENT-CUI-ENG with sensitivity labels. Remote engineers connect "
        ...
        "gov_poc_email": "",
        "hardware_inventory": (
            "In-scope hardware (see CMDB export CMDB-TRIDENT-2026-Q1): "
        ...
        "hw_sw_org_owned": "Yes — all in-scope hardware is owned by Trident; cloud services are contractor-operated GCC subscriptions under Trident tenant.",
        "header_short_name": "Trident Defense Mfg",
    }
...
def _met_narrative(cid: str, info: dict) -> str:
    ...
    template = _FAMILY_NARRATIVE.get(
        family,
        (
            "Trident addresses {name} through documented policies and technical controls in the CUI enclave. "
...
def get_demo_answers() -> Dict[str, Any]:
    ...
    ev_map, _ = _demo_evidence_map()
    for cid, ev_list in ev_map.items():
        answers[cid]["evidence"] = ev_list

    from guidance.objective_guidance import OBJECTIVE_DELIVERABLE_GUIDANCE

    for cid, ans in answers.items():
        raw_objs = OFFICIAL_OBJECTIVES.get(cid, [])
        if not raw_objs:
            continue
        status = ans.get("status", "NOT STARTED").upper().strip()
        objectives = []
        if status == "MET":
            for letter, text in raw_objs:
                objectives.append({"letter": letter, "text": text, "status": "MET"})
        elif cid == "AU.L2-3.3.1":
            for letter, text in raw_objs:
                s = "MET" if letter in ("c", "d") else "NOT_STARTED"
                objectives.append({"letter": letter, "text": text, "status": s})
        elif cid == "IA.L2-3.5.3":
            for letter, text in raw_objs:
                s = "MET" if letter in ("a", "b") else "NOT_STARTED"
                objectives.append({"letter": letter, "text": text, "status": s})
        elif cid == "IR.L2-3.6.1":
            for letter, text in raw_objs:
                s = "MET" if letter in ("a", "b", "c") else "NOT_STARTED"
                objectives.append({"letter": letter, "text": text, "status": s})
        elif cid == "SC.L2-3.13.11":
            for letter, text in raw_objs:
                objectives.append({"letter": letter, "text": text, "status": "NOT_STARTED"})
        elif cid == "CM.L2-3.4.1":
            for letter, text in raw_objs:
                s = "MET" if letter in ("a", "d") else "NOT_STARTED"
                objectives.append({"letter": letter, "text": text, "status": s})
        else:
            for letter, text in raw_objs:
                objectives.append({"letter": letter, "text": text, "status": "NOT_STARTED"})
...
def _demo_appendix_bytes() -> Dict[str, bytes]:
    irp = (
        "TRIDENT DEFENSE SYSTEMS — INCIDENT RESPONSE PLAN (EXCERPT / DEMO)\n"
        ...
        "2. Reporting\n"
        "Users report to helpdesk@tridentdefense.example within 15 minutes. ISO assesses within 1 hour. "
    ...
    acm = (
        "TRIDENT CUI ACCESS CONTROL MATRIX (EXCERPT / DEMO)\n"
        "SharePoint site: TRIDENT-CUI-ENG\n\n"
    ...
    return {
        "appendix:Trident_IR_Plan_Draft.txt": irp.encode("utf-8"),
        "appendix:Trident_Access_Control_Matrix.txt": acm.encode("utf-8"),
    }
...
def get_demo_org_assets() -> Dict[str, Any]:
    return {
        ...
        "appendix_files": [
            {"filename": "Trident_IR_Plan_Draft.txt", "label": "Incident Response Plan v2.1 (draft excerpt)"},
            {"filename": "Trident_Access_Control_Matrix.txt", "label": "Access Control Matrix — CUI SharePoint"},
        ],
    }
...
def get_demo_trident_session_payload() -> Dict[str, Any]:
    _, restored = _demo_evidence_map()
    return {
        "org_name": DEMO_ORG_NAME,
